...
for specific_par in specific_pars:
    ...
    for q in qV[2:]:
        ...
        if q <= q_1:
            ...
            deltap_lo = calc_delta_p_leader_driven(parD, lm, partns, M, q, p_sing)
            deltap_hi = calc_delta_p_leader_driven(parD, lm, partns, M, q, 1-1e-6)

            if np.sign(deltap_lo) != np.sign(deltap_hi):
                ps = bisect(lambda p: calc_delta_p_leader_driven(parD, lm, partns, M, q, p), p_sing, 1-1e-6)
            else:
                ps = np.nan
        ...
        if q <= q_0:
            ...
            deltap_hi = calc_delta_p_leader_driven(parD, lm, partns, M, q, p_sing)

            if np.sign(deltap_lo) != np.sign(deltap_hi):
                ps = bisect(lambda p: calc_delta_p_leader_driven(parD, lm, partns, M, q, p), 1e-6, p_sing)
            else:
                ps = np.nan
