...
for n, tau in zip(nV, tauV):
    ...
    if delta_p_at_q1 > 0:
        ...
        while abs(q_lo-q_hi) > 1e-4:
            ...
            try:
                p_up = bisect(lambda p: calc_delta_p_leader_driven(parD, lm, partns, M, q_mid, p), 
                        p_mid, 1-1e-6)
            except:
                p_up = np.nan

            try:
                p_lo = bisect(lambda p: calc_delta_p_leader_driven(parD, lm, partns, M, q_mid, p), 
                        1e-6, p_mid)
            except:
                p_lo = np.nan
...
plt.tight_layout()
plt.savefig('../../results/leader_driven/critical_v_n' + suffix + '.pdf')
plt.close()

# notes to self for limit checking
# In [4]: q0V[-1]
# Out[4]: 0.5334450681033289
# In [5]: q1V[-1]
# Out[5]: 0.489731965780894

# when n = 24, tau = 12
# In [10]: q_0
# Out[10]: 0.5203338515778323
# In [12]: q_1
# Out[12]: 0.4870134116280195
