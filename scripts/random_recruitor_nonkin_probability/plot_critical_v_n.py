...
for n, tau in zip(nV, tauV):
    ...
    if delta_p_at_q1 > 0:
        ...
        while abs(q_lo-q_hi) > 1e-4:
            ...
            try:
                p_up = bisect(lambda p: calc_delta_p_random_probability(parD, lm, partns, M, q_mid, p), 
                        p_mid, 1-1e-6)
            except:
                p_up = np.nan

            try:
                p_lo = bisect(lambda p: calc_delta_p_random_probability(parD, lm, partns, M, q_mid, p), 
                        1e-6, p_mid)
            except:
                p_lo = np.nan
...
plt.tight_layout()
plt.savefig('../../results/random_recruitor_nonkin_probability/critical_v_n' + suffix + '.pdf')
plt.close()
