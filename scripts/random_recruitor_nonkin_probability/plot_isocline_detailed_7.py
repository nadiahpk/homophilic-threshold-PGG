...
suffix = '_7'   # which default parameter values to use from the file default_params.csv
...
make_arrow(q_1/2,         0.55, -0.1)
make_arrow((q_0+q_1)/2,   0.51, -0.1)
make_arrow((q_0+q_1)/2,   0.90, +0.07)
make_arrow((q_0+q_max)/2, 0.43, -0.1)
make_arrow((q_0+q_max)/2, 0.03, +0.1)
make_arrow((q_0+q_max)/2, 0.75, +0.1)
...
plt.tight_layout()
plt.savefig('../../results/random_recruitor_nonkin_probability/isocline_detailed' + suffix + '.pdf')
plt.close()
