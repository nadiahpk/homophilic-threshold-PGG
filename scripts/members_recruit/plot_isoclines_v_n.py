...
specific_pars = [
        {'n': 10, 'tau': 5, 'colour': 'red'},
        {'n': 8,  'tau': 4, 'colour': 'blue'},
        {'n': 6,  'tau': 3, 'colour': 'orange'},
        {'n': 4,  'tau': 2, 'colour': 'black'}
        ]
...
for specific_par in specific_pars:
    ...
    for q in qV[2:]:
        ...
        if q <= q_1:
            ...
            deltap_lo = calc_delta_p_members_recruit(parD, lm, partns, M, q, p_sing)
            deltap_hi = calc_delta_p_members_recruit(parD, lm, partns, M, q, 1-1e-6)

            if np.sign(deltap_lo) != np.sign(deltap_hi):
                ps = bisect(lambda p: calc_delta_p_members_recruit(parD, lm, partns, M, q, p), p_sing, 1-1e-6)
            else:
                ps = np.nan
        ...
        if q <= q_0:
            ...
            deltap_hi = calc_delta_p_members_recruit(parD, lm, partns, M, q, p_sing)

            if np.sign(deltap_lo) != np.sign(deltap_hi):
                ps = bisect(lambda p: calc_delta_p_members_recruit(parD, lm, partns, M, q, p), 1e-6, p_sing)
            else:
                ps = np.nan
...
for qV, p_stableV, p_unstableV, q_0, q_1, q_max, p_sing, s_par in zip(qM, p_stableM, p_unstableM, q_0V, q_1V, q_maxV, p_singV, specific_pars):
    ...
    l1, = plt.plot(1-np.array(qV), p_stableV,   color=colour, alpha=0.7, lw=3)
    l2, = plt.plot(1-np.array(qV), p_unstableV, color=colour, alpha=0.7, ls='dashed')
    ...
    plt.plot([0, 1-q_0], [0, 0],  color=colour, alpha=0.7, lw=3)
    plt.plot([0, 1-q_1], [1, 1],  color=colour, alpha=0.7, ls='dashed')

    # plot the point where the transition from 2 to 0 interior equilibria occurs (if needed)
    if q_max < 1:
        plt.scatter([1-q_max], [p_sing], color=colour)
...
legend2 = plt.legend([l[0] for l in plot_lines], [r'$n=' + str(spar['n']) + r'$' for spar in specific_pars], loc='center right')
...
plt.xlabel(r'homophily $(1-q)$', fontsize='x-large')
plt.ylabel(r'proportion of Cooperators $p$', fontsize='x-large')
plt.xlim((0, 1))
plt.ylim((-0.05, 1.12))

plt.tight_layout()
plt.savefig('../../results/members_recruit/isoclines_v_n.pdf')
plt.close()
